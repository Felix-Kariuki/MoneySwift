...
import com.flexcode.ecommerce.domain.utils.parseJson
 ...
 * @property context
 */
class GetListingsRepositoryImpl @Inject constructor(
    ...
    override suspend fun getListings(): Result<List<ListingsResponse>> {
        return try {
            ...
            Result.success(response.map(ListingsResponseDto::toDomain))
        } catch (e: Exception) {
            e.printStackTrace()
            Result.failure(e)
        }
